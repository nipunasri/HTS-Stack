...
UINT CFrontEnd::uiBuildOCLKernels()
{
  ...
  for(UINT i = 0; i < OCL_HASH_TABLE_SIZE; ++i) // Added --> Nipuna
    {
  ...
  pMiscData->uiHeadIndex = OCL_HASH_TABLE_SIZE ; // Added --> Nipuna
  ...
  for(cl_uint i = OCL_HASH_TABLE_SIZE; i < uiEndIndex; ++i) 
    {
  ...
  pNodePool[1025].uiNext = SET_PTR(1026) ;
  pNodePool[1026].uiNext = 0 ;

  for(int i = 0 ; i <= 255; i++) {
	 pNodePool[1025].pE[i] = 1024*(i+1) ;
  }
  for(int i = 0; i <= 255; i++) { 
    pNodePool[1026].pE[i] = 1024*(257+i) ;
  }
  pNodePool[1024].uiNext = SET_PTR(1027) ;
#endif
  //create the program
  ...
  if(uiFlags & HTS_REQ_COMPLETED)
    {
      ...
      *pStatus   = uiRStatus;
	  ...
	  int temp = GET_PTR(pNodePool[0].uiNext) ;

	  /*for(int i = 0; i <= 255; i++)
		  std::cout<<(pNodePool[temp].pE[i])<<" " ;
	  std::cout << std::endl ;*/
